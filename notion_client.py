...
def create_page(database_id: str, title: str, children: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # Real POST — an API key was supplied, so the caller wants a real upload.
    # A missing dependency is a setup error, not a network blip: let it raise
    # rather than hand back a stub that reads as success.
    import requests
